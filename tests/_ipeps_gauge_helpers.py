...
# --- the externally-derived chain spectra ----------------------------------
#
# Kept here rather than in ``test_ipeps_gauge.py`` (#882 final review, P3-15):
# ``test_ipeps_su.py`` needs both of them, and importing one ``test_`` module
# from another is exactly the practice this helper module was created to
# avoid -- see the docstring above.  They are certified *outside* tenax, so a
# second copy would be a second thing that can go stale; there is one, and it
# is here.
