...
def _monolith_right(T2, a, P1, P2, chi, D2):
    ...
    Tg = T2a.transpose(0, 2, 1, 3, 4).reshape(
        chi * D2, chi * D2, D2
    )  # (fl=(t2_u,u2), fr=(t2_d,d2), l2)
...
def test_chunked_left_complex_projector():
    ...
    T4 = jax.random.normal(keys[0], (chi, D2, chi)) + 1j * jax.random.normal(
        keys[1], (chi, D2, chi)
    )
    a = jax.random.normal(keys[2], (D2, D2, D2, D2)) + 1j * jax.random.normal(
        keys[3], (D2, D2, D2, D2)
    )
    P1 = jax.random.normal(keys[4], (chi * D2, chi)) + 1j * jax.random.normal(
        keys[5], (chi * D2, chi)
    )
    P2 = jax.random.normal(keys[6], (chi * D2, chi)) + 1j * jax.random.normal(
        keys[7], (chi * D2, chi)
    )
    ref = _monolith_left(T4, a, P1, P2, chi, D2)
...
def test_full_left_move_chunked_matches_default():
    ...
    for field in ("C1", "C4", "T4"):
        ...
        rel = float(
            jnp.max(jnp.abs(c_arr_aligned - b_arr)) / (jnp.max(jnp.abs(b_arr)) + 1e-30)
        )
...
def test_converge_1x1_chunk_matches_default():
    ...
    site_tensors = {(0, 0): A}
    neighbors = {
        (0, 0): {direction: (0, 0) for direction in ("left", "right", "top", "bottom")}
    }
    ...
    envs_on, _ = python_loop_ctm_converge(
        site_tensors, neighbors, ctm_chunk_size=3, **kw
    )
    ...
    for field in e_off._fields:
        ...
        rel = float(
            jnp.max(jnp.abs(cc - b.todense())) / (jnp.max(jnp.abs(b.todense())) + 1e-30)
        )
