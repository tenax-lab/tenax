...
- **The iPEPS site tensor's physical leg must be labelled `phys`.** With any other label `compute_energy_ctm_tensor` raises `ValueError: output_labels contains 'phys' which is not a free label`. `heisenberg_gate()` (dense) and `heisenberg_gate_u1sz()` (symmetric) give the same energy on a symmetric env.

  **CORRECTION:** an earlier revision quoted `E = -0.114511058317` as the fresh-env smoke reference. That number was measured **before** the phys-last finding, i.e. on a wrongly-ordered tensor, and does not reproduce. The correct value is `-0.019134242178`. Do not use the old number for anything.

- **The U(1) `_site_tensor()` does not converge — and neither does it in the DENSE module.** Measured: residual 1.0-1.9 over 200 sweeps, energy drifting 0.31 -> 1.17 -> 2.32 at 60/100/200. Its U(1) structure (virtual `[0,1]` against physical `[-1,1]`) admits only 8 scalar blocks. A search over 112 variants (every single- and pair-block boost x 3 noise levels x 2 seeds), plus U(1) at D=3 with 32 blocks, found **zero** convergent cases. So this is a property of the state, not of the port.

  Consequence for the plan's testing strategy: anything needing a converged limit (Tasks 7-10) uses `_convergent_site_tensor()` — Z2, D=2, converges in 36 sweeps, symmetric and dense alike. The U(1) fragmenting layout is still covered, but by **sweep-by-sweep parity**, which needs no limit. This preserves the design's requirement to exercise unequal sector sizes while not pretending a non-convergent state converges. Cf. [[feedback_ctm_parity_needs_convergent_input]], which is the same lesson: parity tests need convergent inputs or the numbers are meaningless.
