...
- **Check what your `gh` actually supports before building a watcher on it, and make the watcher fail *closed*.** The version moves under you, so do not memorise one — including the one in this sentence. This clone was on 2.45.0, where `gh pr checks --json` exits with `unknown flag: --json`, and a watcher built on it hits the error branch every poll and can never report (one ran 11 hours in a single session); as of 2026-08-14 it is 2.93.0, where `--json` works and returns `state` per check. Run `gh --version` and `gh pr checks --help` and believe those, not this line. Whatever you poll, exit only on an explicit success condition — `gh pr view <n> --json state --jq .state` returning `MERGED`/`CLOSED` — so a transient `gh` error retries instead of falling out of the loop as though it had succeeded.
