...
- **Read the review comments before merging — green CI is not review.** Codex reviews every PR, and its findings arrive *after* the checks go green, so a PR can be `CLEAN` and merge-ready while carrying an unread P1. Check both endpoints, since inline comments do not appear in the reviews list:
  ...
  ```
  gh api repos/tenax-lab/tenax/pulls/<n>/reviews  --jq '.[] | "[\(.state)] \(.user.login)\n\(.body)"'
  gh api repos/tenax-lab/tenax/pulls/<n>/comments --jq '.[] | "\(.path):\(.line)\n\(.body)"'
  ```
...
- **`gh pr checks --json` is not supported in this `gh` version.** A watcher built on it silently retries forever and can never report; use the text output, or poll state with `gh pr view <n> --json state --jq .state` and exit only on an explicit `MERGED`/`CLOSED` so a transient `gh` error retries instead of falling out of the loop as if it had succeeded.
